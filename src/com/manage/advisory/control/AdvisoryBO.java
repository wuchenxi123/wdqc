 ...
 * @version 1.0
 */
public class AdvisoryBO extends AbstractControlBean implements Advisory {

	public AdvisoryVO doCreate(AdvisoryVO vo) throws Exception {
		try {
			AdvisoryDAO dao = (AdvisoryDAO) DAOFactory.build(AdvisoryDAO.class,
					user);
			// TODO set the pk */
	...
	public DataPackage doQuery(AdvisoryDBParam params) throws Exception {
		AdvisoryDAO dao = (AdvisoryDAO) DAOFactory.build(AdvisoryDAO.class,
				user);
	...
	private AdvisoryVO fillCourse(AdvisoryVO vo) throws Exception {
		...
		if (vo.getAdCourse() != null) {
			CourseVO c = (CourseVO) dao.findByPk(Integer.valueOf(vo
					.getAdCourse()));
	...
	private AdvisoryVO fillUser(AdvisoryVO vo) throws Exception {
		...
		if (vo.getCreator() == null) {
			...
			MemberVO c = (MemberVO) dao.findByPk(Integer.valueOf(vo
					.getCreator()));
